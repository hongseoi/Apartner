...
<?php
 ...
 $price_arr = array();
 $date_arr = array();
  $total_arr = array(['yyyymm', '(단위:만원)']);

 while($row = mysqli_fetch_array($ret)){
  array_push($price_arr,$row['price']);
  array_push($date_arr, $row['YYYY']);
  array_push($total_arr, [(double)$row['YYYY'], (double)$row['price']]);
...
<div class="chart1">
<div id="linechart_div" class='chart' style="width:100%; height: 100%;"></div>    
</div> 
...
<thead>
<tr>
<th colspan="2" scope="cols"><h1>관련 기사</h1></th>
</tr>
</thead>
<tbody>
  ...
  <?php
    $wantedcode=getenv("QUERY_STRING");
    $con=mysqli_connect("localhost", "root", "000000", "aptinfo") or die("MySQL 접속 실패");
    $sql = " SELECT * FROM news_info where apart_code='$wantedcode';";
